...
# Author: guyezi<https://guyezi.com>

echo "#############################################################"
echo "#         CentOS 7/8 v2ray 带伪装一键安装脚本                 #"
echo "# 网址: https://guyezi.com                                  #"
echo "# 作者: guyezi                                                #"
echo "#############################################################"
echo ""
...
http://ssr.guyezi.com/
http://web.guyezi.com/
https://www.guyezi.com/
http://sss.guyezi.com/
https://web.guyezi.com/
)
...
{
    echo 安装v2ray...
    bash <(curl -sL https://raw.githubusercontent.com/guyezi/install-v2ray/master/go_v2r.sh)

    if [ ! -f /etc/v2ray/config.json ]; then
        echo "安装失败，请到 https://guyezi.com 网站反馈"
        exit 1
    fi
...
function installNginx()
{
    ...
    res=`which pip3`
    if [ "$?" != "0" ]; then
        echo -e " pip3安装失败，请到 ${red}https://www.guyezi.com${plain} 反馈"
        exit 1
    fi
    ...
    if [ "$?" != "0" ]; then
        echo -e " 获取证书失败，请到 ${red}https://www.guyezi.com${plain} 反馈"
        exit 1
    fi
...
EOF
    ...
    if [ "${res}" = "" ]; then
        echo -e "nginx启动失败！ 请到 ${red}https://www.guyezi.com${plain} 反馈"
        exit 1
    fi
}
