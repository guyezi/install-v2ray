...
installSoftware(){
    ...
    if [[ $SOFTWARE_UPDATED -eq 0 ]]; then
        colorEcho ${BLUE} "更新软件 repo"
        $CMD_UPDATE
        SOFTWARE_UPDATED=1
    fi

    colorEcho ${BLUE} "${COMPONENT} 安装中"
    $CMD_INSTALL $COMPONENT
    if [[ $? -ne 0 ]]; then
        colorEcho ${RED} "未能安装 ${COMPONENT}. 请手动安装."
        return 1
    fi
    return 0
}
...
# 1: 新版 V2Ray. 0: no. 2: 未安装. 3: 检查失败. 4: 不要检查.
getVersion(){
    if [[ -n "$VERSION" ]]; then
        ...
        if [[ $? -ne 0 ]] || [[ $NEW_VER == "" ]]; then
            colorEcho ${RED} "无法获取发布信息. 请检查您的网络与重试."
            return 3
...
stopV2ray(){
    ...
    if [[ $? -ne 0 ]]; then
        colorEcho ${YELLOW} "关闭 V2Ray 服务失败."
        return 2
    fi
    return 0
}
...
startV2ray(){
    ...
    if [[ $? -ne 0 ]]; then
        colorEcho ${YELLOW} "启动 V2Ray 服务失败."
        return 2
    fi
    return 0
}
...
installV2Ray(){
    ...
    chmod +x '/usr/bin/v2ray/v2ray' '/usr/bin/v2ray/v2ctl' || {
        colorEcho ${RED} "复制 V2Ray 二进制程序和资源失败."
        return 1
    }
    ...
    if [ ! -f '/etc/v2ray/config.json' ]; then
        ...
        '/etc/v2ray/config.json' || {
            colorEcho ${YELLOW} "创建 V2Ray 配置文件失败. 请手动创建."
            return 1
        }
...
installInitScript(){
    if [[ -n "${SYSTEMCTL_CMD}" ]] && [[ ! -f "/etc/systemd/system/v2ray.service" && ! -f "/lib/systemd/system/v2ray.service" ]]; then
        ...
        wget -O /etc/systemd/system/v2ray.service https://raw.githubusercontent.com/guyezi/install-v2ray/master/v2ray.service
...
./install-release.sh [-h] [-c] [--remove] [-p proxy] [-f] [--version vx.y.z] [-l file]
  -h, --help            显示帮助
  -p, --proxy           通过代理服务器下载, 用法 -p socks5://127.0.0.1:1080 or -p http://127.0.0.1:3128 etc
  -f, --force           强制安装
      --version         安装特定版本, 用法 --version v3.15
  -l, --local           从本地文件安装
      --remove          卸载已安装的 V2Ray
  -c, --check           检查更新
EOF
...
remove(){
    if [[ -n "${SYSTEMCTL_CMD}" ]] && [[ -f "/etc/systemd/system/v2ray.service" ]];then
        ...
        if [[ $? -ne 0 ]]; then
            colorEcho ${RED} " V2Ray 卸载失败."
            return 0
        else
            colorEcho ${GREEN} "卸载 V2Ray 成功."
...
checkUpdate(){
    ...
    RETVAL="$?"
    if [[ $RETVAL -eq 1 ]]; then
        colorEcho ${BLUE} "为 V2Ray 找到新版本 ${NEW_VER} .(当前版本:$CUR_VER)"
    elif [[ $RETVAL -eq 0 ]]; then
        colorEcho ${BLUE} "没有新版本. 当前版本为 ${NEW_VER}."
    elif [[ $RETVAL -eq 2 ]]; then
        colorEcho ${YELLOW} "未安装 V2Ray ."
        colorEcho ${BLUE} " V2Ray 的最新版本是 ${NEW_VER}."
    fi
    return 0
}
...
main(){
    ...
    if [[ $LOCAL_INSTALL -eq 1 ]]; then
        colorEcho ${YELLOW} "通过本地文件安装 V2Ray . 请确保文件是一个有效的 V2Ray 资源包, 因为我们无法确定."
        NEW_VER=local
        rm -rf /tmp/v2ray
        ZIPFILE="$LOCAL"
        ...
        #    colorEcho ${RED} "本地 V2Ray 不能在linux中安装."
        #    return 1
